globals [
  initial-trees
  ;; how many trees (green or pink) we started with
]

;; Egemen Öner / 18070006024 / SE 3360 Assignment # 1
...
to setup
  ...
  ask patches [
    ...
    [ set pcolor green ]
    if (random 150) < density
    [ set pcolor pink ]
    if pxcor = min-pxcor
    [ set pcolor red ]
]
  set initial-trees count patches with [pcolor = green or pcolor = pink]
  reset-ticks
end
...
to go
  ...
  ;; each burning tree (red patch) checks its "8" neighbors.
  ...
  ask patches with [pcolor = red] [
    ...
    ask neighbors ; Q1 modification
    with [pcolor = green or pcolor = pink] [ ; Q4 modification

      let probability probability-of-spread
      let probabilitypink pink-probability-of-spread
      ...
      if (direction = 0 ) [
        ...
        set probabilitypink probabilitypink - south-wind-speed
      ]
       ;---------------------------------------------------------
      ...
      if (direction = 90 ) [
        ...
        set probabilitypink probabilitypink - west-wind-speed
      ]
      ;---------------------------------------------------------
      ...
      ;; As I mentioned in report, in q3, there is 2 possible way to sparks land in random location,
      ;; I used both ways in 2 types of trees to try them

      if random 100 < probabilitypink and pcolor = pink [
        set pcolor red ;; to catch on fire
        if big-jumps? and random 100 < probabilitypink ; Q2 Modification
        [
         ; let target one-of patches in-radius 5 ; Q3 Modification
         let target patch-at (west-wind-speed / (random 6 + 6)) (south-wind-speed / (random 6 + 6)) ; Q3 Modification
          if target != nobody and [ pcolor ] of target = pink [
            ask target [
      ...
      if random 100 < probability and pcolor = green [
        set pcolor red ;; to catch on fire
        if big-jumps? and random 100 < probability ; Q2 Modification
        [
          let target one-of patches in-radius 5 ; Q3 Modification
         ; target patch-at (west-wind-speed / (random 6 + 6)) (south-wind-speed / (random 6 + 6)) ; Q3 Modification
