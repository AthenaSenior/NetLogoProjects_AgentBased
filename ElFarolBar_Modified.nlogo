...
globals [
  ...
  memory-size       ;; Memory size is no longer a slider, only modified inside code
...
turtles-own [
  ...
  successRate     ;; Success Rate of Agents
  successfulPredictions ;; Agents may have successfull predictions
]
...
to setup
  ...
  ask bar-patches [ set pcolor brown ]
  ...
  ; n-values memory-size * 2 [random 100]:
  ; (If memory size is 5) => [10 43 23 13 02 91 84 92 72 15] History
  ;first: 0th element in list: attendance becomes first element of history list
...
to go
  ...
  ask turtles [
    ...
    set color scale-color blue reward  (max [ reward ] of turtles + 1) 0
    ; show prediction
    ]
  ...
  ;; IN EACH TICK, WE TAKE THE SUCCESSFUL PREDICTIONS:
 if ticks = 0
  [
    ;show turtles with [attend?]
    ask turtles[
      if (attendance < overcrowding-threshold) and (prediction < overcrowding-threshold) and attend?
      [
        set successfulPredictions successfulPredictions + 1
      ]
    ]
  ]
  ...
  [
    ...
    set successRate successfulPredictions / 10
    ]
    ask turtles[
      if successRate < 0.3
      [
        set strategies n-values number-strategies [random-strategy] ; Re-initialize weights randomly
      ]
    ]
  ]
...
to update-strategies
  ...
  foreach strategies [ the-strategy ->
    ...
    [
      ; set week week + 1
      set prediction predict-attendance the-strategy ; FIRST PARAMETER
      sublist history week (week + memory-size) ; SECOND PARAMETER
      ; Going back only 1 week
...
to-report random-strategy
  ...
  ; n-values (memory-size + 1) [1.0 - random-float 2.0]:
  ; (If memory size is 5) => [-0.5 0.0 -1.0 1.0 -0.3 0.4] Strategy
end
...
to-report predict-attendance [strategy subhistory]
  ...
  ;; p(t) =
  report 100 * first strategy ; c*100
  + (sum(map [ [week weight] -> weight * week ] butfirst strategy subhistory)) / 5 ; Q1C
  ; + sum(map [ [week weight] -> weight * week ] butfirst strategy subhistory) (Q1A Q1B)
end
